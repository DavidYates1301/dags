...
BATCH_SIZE = 1000
MAX_TOTAL_ROWS = 100000  # Giới hạn tổng số bản ghi
...
def get_total_batches():
    ...
    total_rows_in_table = hook.get_first(sql)[0]
    print(f"Total rows in source table: {total_rows_in_table}")

    total_rows = min(total_rows_in_table, MAX_TOTAL_ROWS)
    ...
    print(f"Prepared {len(task_inputs)} batches for copy.")
    return task_inputs
...
with DAG(
    dag_id="trino_copy_limited_batches",
